...
while 1:
    ...
    V_n = V.copy()
    for s in estados:
        V[s] = min(1 + l*sum(P[accion][s][j]*V[s] for j in estados) for accion in x)
    if normaInfinito(V_n,V) <= 1e-5 or iter == 10000:
        break

for s in estados:
    d[s] = 1 + np.argmin([1 + l*sum(P[accion][s][j]*V[s] for j in estados) for accion in x])
